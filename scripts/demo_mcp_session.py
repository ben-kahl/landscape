...
Connects to a running Landscape FastAPI server over streamable HTTP and drives a
7-step session:
...
Run:
    # Start the server first (docker stack must be up):
    uv run uvicorn landscape.main:app --host 0.0.0.0 --port 8000

    # Then in another terminal:
    uv run python scripts/demo_mcp_session.py

    # Override server URL:
    LANDSCAPE_MCP_URL=http://localhost:8000/mcp uv run python scripts/demo_mcp_session.py
"""
